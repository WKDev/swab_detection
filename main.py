...
def hough_circles_operation(image, param1, param2, min_radius, max_radius):
    global hough_result
    hough_result = cv2.HoughCircles(image, cv2.HOUGH_GRADIENT, 1, 75,
...
while True:
    ...
    try:
        adaptive_thresh_image = cv2.adaptiveThreshold(clahed_gray, max_value, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, block_size, c)
        thresh_bin_image = cv2.threshold(clahed_gray, threshold_min, threshold_max, cv2.THRESH_BINARY)[1]
        thresh_otsu_image = cv2.threshold(clahed_gray, threshold_min, threshold_max, cv2.THRESH_OTSU)[1]
        ...
        bin_image = adaptive_thresh_image
    ...
    hough_thread = threading.Thread(target=hough_circles_operation, 
                                    args=(adjusted, param1, param2, min_radius, max_radius))
    hough_thread.start()
    ...
    for k, v in processed_imgs.items():
        ...
        processed_imgs[k] = cv2.putText(processed_imgs[k], k, (50, processed_imgs[k].shape[1]-100), cv2.FONT_HERSHEY_SIMPLEX, 5, (0, 255, 0), 10)
